...
export class SettlementDriveClient {
  ...
  async uploadResumableChunk(input: {
    ...
    expectedParentId: string;
    expectedAppProperties: SettlementDriveAppProperties;
  }): Promise<{ complete: false; nextOffsetBytes: number } | { complete: true; metadata: SettlementDriveFileMetadata }> {
    const operation = "uploadResumableChunk";
    validateMimeType(input.mimeType, operation);
    validateSize(input.offsetBytes, operation);
    validateSize(input.totalSizeBytes, operation);
    validateDriveId(input.expectedParentId, operation);
    const expectedProperties = normalizeAppProperties(input.expectedAppProperties, operation)!;
    if (!(input.bytes instanceof Uint8Array)
      || input.bytes.byteLength < 1 || input.bytes.byteLength > MAX_UPLOAD_CHUNK_BYTES
      || input.offsetBytes >= input.totalSizeBytes
      || input.offsetBytes + input.bytes.byteLength > input.totalSizeBytes) throw failure(operation);
    const final = input.offsetBytes + input.bytes.byteLength === input.totalSizeBytes;
    if (!final && input.bytes.byteLength % (256 * 1024) !== 0) throw failure(operation);
    const url = new URL(validateSessionUrl(input.sessionUrl, operation, 0));
    let token: string;
    try { token = await this.getAccessToken(); } catch { throw failure(operation); }
    if (typeof token !== "string" || !ACCESS_TOKEN_PATTERN.test(token)) throw failure(operation);
    ...
    try {
      response = await this.fetchImpl(url, {
        ...
        headers: {
          ...
          "Content-Type": input.mimeType, "Content-Length": String(input.bytes.byteLength),
          "Content-Range": `bytes ${input.offsetBytes}-${input.offsetBytes + input.bytes.byteLength - 1}/${input.totalSizeBytes}`,
        },
        body: input.bytes as BodyInit, redirect: "error",
      });
    } catch { throw failure(operation); }
    if (response.status === 308) {
      const expectedEnd = input.offsetBytes + input.bytes.byteLength - 1;
      const range = response.headers.get("Range");
      await cancelResponseBody(response);
      if (range !== `bytes=0-${expectedEnd}`) throw failure(operation, response.status);
      return { complete: false, nextOffsetBytes: expectedEnd + 1 };
    }
    if (response.status !== 200 && response.status !== 201) {
      await cancelResponseBody(response); throw failure(operation, response.status);
    }
    if (!final) { await cancelResponseBody(response); throw failure(operation, response.status); }
    const metadata = validateMetadata(await parseJson(response, operation), operation, response.status);
    if (metadata.size !== String(input.totalSizeBytes) || metadata.mimeType !== input.mimeType
      || !isExactParent(metadata.parents, input.expectedParentId)
      || !sameAppProperties(metadata.appProperties, expectedProperties)) throw failure(operation, response.status);
